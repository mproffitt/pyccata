""" Test module for releaseessentials.document """
import os
...
class TestDocumentController(TestCase):
    ...
    @patch('releaseessentials.manager.Manager._load')
    ...
    @patch('releaseessentials.configuration.Configuration._parse_flags')
    @patch('releaseessentials.document.DocumentController.threadmanager')
    @patch('releaseessentials.manager.Manager._load')
    @patch('releaseessentials.configuration.Configuration._get_locations')
    @data(
        (ImportError, 'Failed to import module \'idonotexist\''),
    ...
    def test_report_manager_raises_invalid_module_error_if_parts_module_doesnt_exist(self, mock_config, mock_parser):
        ...
        with self.assertRaises(InvalidModuleError):
            with patch('releaseessentials.factory.DocumentPartFactory.MODULE', new_callable=PropertyMock) as mock_module:
                mock_module.return_value = 'pts'
                DocumentController()

    @patch('releaseessentials.document.DocumentController.reportmanager')
    @patch('releaseessentials.document.DocumentController.threadmanager')
    @patch('releaseessentials.document.DocumentController.configuration')
    ...
    @patch('releaseessentials.configuration.Configuration._parse_flags')
    @patch('releaseessentials.configuration.Configuration._get_locations')
    @patch('releaseessentials.managers.thread.ThreadManager.__init__')
    ...
    def test_document_raises_thread_failed_error_if_threadmanager_execute_returns_false(self, mock_config, mock_parser):
        ...
        document = DocumentController()
        with patch('releaseessentials.managers.thread.ThreadManager.execute') as mock_thread:
    ...
    @patch('releaseessentials.managers.subjects.docx.Docx.format_for_email')
    @patch('releaseessentials.configuration.Configuration._parse_flags')
    @patch('releaseessentials.configuration.Configuration._get_locations')
