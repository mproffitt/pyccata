...
from releaseessentials.parts.list import List
...
class TestList(TestCase):
    ...
    @patch('releaseessentials.managers.report.ReportManager.add_list')
    ...
    def test_run_completes_if_filter_delays_in_completing(self):
        ...
        with patch('releaseessentials.filter.Filter.complete', new_callable=PropertyMock) as mock_thread_complete:
            unordered = List(self._thread_manager, config)
    ...
    def test_run_completes_if_thread_fails(self):
        ...
        with patch('releaseessentials.threading.Threadable.complete', new_callable=PropertyMock) as mock_thread_complete:
            unordered = List(self._thread_manager, config)
    ...
    @patch('releaseessentials.filter.Filter.results', new_callable=PropertyMock)
    @patch('releaseessentials.managers.report.ReportManager.add_list')
    def test_init_using_single_query_creates_list_of_items(self, mock_list, mock_results):
        ...
        with patch('releaseessentials.filter.Filter.complete', new_callable=PropertyMock):
            unordered = List(self._thread_manager, config)
